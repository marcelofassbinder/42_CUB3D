...
void	draw_element_minimap(t_cub *cub, int x, int y, int color)
{
	...
	while (h < cub->minmap_square)
	{
		...
		{
			my_mlx_pixel_put(&cub->image, w + (x * cub->minmap_square),
				h + (y * cub->minmap_square), color);
			w++;
		}
		h++;
	}
	...
	while (x <= sphere_ray)
	{
		...
		while (y <= sphere_ray)
		{
			if (x * x + y * y <= sphere_ray * sphere_ray)
				my_mlx_pixel_put(&cub->image, player_minimap.x + x,
					player_minimap.y + y, 0x032DFD);
			y++;
		}
		x++;
	}
...
void	draw_rays_minimap(t_cub *cub)
{
	...
	while (ray_id <= WIDTH)
	{
		camera_x = 2 * (ray_id / (double)WIDTH) - 1;
...
void	draw_one_ray_minimap(t_cub *cub, t_coordinate *ray_vector)
{
	...
	while (42)
	{
		...
		my_mlx_pixel_put(&cub->image, (ray.x * cub->minmap_square),
			(ray.y) * cub->minmap_square, 0xFFFF66);
		i += 0.1;
	}
}
